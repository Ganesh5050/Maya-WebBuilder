...
import { Plus, Grid3X3, Clock, Star, Settings, Search, ExternalLink } from 'lucide-react';
...
export default function StarredApps() {
  const [activeTab, setActiveTab] = useState('Dashboard');
  const [starredApps, setStarredApps] = useState<App[]>([]);
  const [loading, setLoading] = useState(true);
  const navigate = useNavigate();
  const { user } = useAuth();

  useEffect(() => {
    const loadStarredApps = async () => {
      if (user) {
        try {
          const apps = await databaseService.getStarredApps(user.id);
          setStarredApps(apps);
        } catch (error) {
  ...
  const filteredApps = starredApps;
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="flex-1 p-6">
          <div className="mb-6">
            <div className="flex items-center justify-between mb-4">
              <h1 className="text-2xl font-semibold text-gray-900">Starred apps</h1>
            </div>
            ...
            <div className="mb-6">
              <div className="relative">
                <input
                  type="text"
                  placeholder="Search starred apps..."
            ...
            <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
              {loading ? (
                <div className="px-6 py-12 text-center">
                  ...
                  <p className="text-gray-600">Loading starred apps...</p>
                </div>
                ...
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-medium text-gray-900 mb-2">No starred apps</h3>
                  <p className="text-gray-600 mb-6">Star your favorite apps to see them here</p>
                  <a href="/apps" className="px-4 py-2 font-medium transition-all duration-200 hover:opacity-90 inline-block"
                    style={{
                      backgroundColor: 'rgb(0, 0, 0)',
                      color: 'rgb(255, 255, 255)',
                      borderRadius: '10px',
